...
import io.github.chaosunity.casc.parsing.`type`.{BuiltInType, ClassType, INT, STRING, Type, VOID}
import org.apache.commons.lang3.StringUtils

object TypeResolver {
    def getFromTypeName(typeContext: CASCParser.TypeContext): Type = {
        if (typeContext == null) return VOID
    ...
    def getFromValue(value: String): Type = {
        if (StringUtils.isEmpty(value)) return VOID
        if (StringUtils.isNumeric(value)) return INT
